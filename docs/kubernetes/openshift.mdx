...
If cert-manager takes longer than 90 seconds to issue certificates, increase the job timeout with `--set pkiInitJob.timeoutSeconds=<seconds>`. The hook polls for `(timeoutSeconds - 30)` seconds, reserving 30 seconds for ConfigMap creation and cleanup. By default (`pkiInitJob.failOnTimeout=true`), the install fails if the timeout is reached, providing clear feedback that the BackendTLSPolicy is non-functional.
