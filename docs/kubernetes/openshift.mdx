...
To use cert-manager instead of the built-in PKI, add `--set certManager.enabled=true` and configure `certManager.serverDnsNames` with the external hostname. On first install with cert-manager, the backend CA ConfigMap is created on the first `helm upgrade` after cert-manager issues the server certificate.
